...
mean = np.array([0.016424339322862132,  -6.183413547227858e-05, -5.117908838102584e-05, 0.002890023486356235,
                 16.861477077370584, 0.6757187611028985, 0.6219754167136202])

std = np.array([ 0.04558777765592853, 0.004756503583955391, 0.004895126171637657, 0.0023049672304388032,
                 6.967605484612514, 0.18184540306728297, 0.17276582239324934])
...
def eval(args,config,):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            # 将三值图像转换为彩色图像并保存
            #convert_to_color(saved_1, r'F:\net\eval_output\eval_ouput_color/{}.tif'.format(path))
        ...
        precision = precision_score(correct, preds, average='weighted')
        recall = recall_score(correct, preds, average='weighted')
        f1 = f1_score(correct, preds, average='weighted')
...
parser.add_argument('--checkpoint',type=str, default=r"G:\net\deeplab+7_yuanshi_Adam_lr=0.0001\DeepLab-0.9444-ep18.pth", help='checkpoint path') # 加载定模型路径
